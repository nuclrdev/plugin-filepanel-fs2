...
/**
 * Swing {@link MoveEngine.ConflictResolver} that shows the "File already exists" warning and, when
 * the user picks Rename, the rename prompt. Honours the "Remember choice" checkbox by caching the
 * answer and applying it to subsequent clashes without re-prompting.
 *
 * <p>One instance is created per move operation so the remembered choice is scoped to that run.
 * Methods marshal to the EDT and block for the user's answer, so the resolver is safe to call from
 * the background move thread. Mirrors the copy conflict dialog.
 */
@Slf4j
final class MoveConflictDialog implements MoveEngine.ConflictResolver {
	...
	private Path promptRename(Window owner, Path target) {
		...
		JTextField field = new JTextField(MoveEngine.autoRename(target).toString(), 40);
	...
	private static void runOnEdtAndWait(Runnable runnable) {
		...
		try {
			...
			log.warn("Failed to run move conflict dialog on EDT: {}", e.getMessage(), e);
		}
	}
}
