...
/**
 * Modal rename/move progress dialog with a Cancel button. The supplied {@code work} runs on a
 * background virtual thread and receives a {@link NuclrPluginCallback} wired to this dialog; the
 * modal dialog itself keeps the EDT pumping (so conflict prompts shown from {@code work} stay
 * responsive) and blocks the caller until the move finishes or is cancelled. Mirrors the copy
 * progress dialog.
 *
 * <p>Must effectively be driven from the EDT; {@link #run} marshals there itself.
 */
@Slf4j
final class MoveProgressDialog {

	private MoveProgressDialog() {
	}
	...
	private static void show(Consumer<NuclrPluginCallback> work, NuclrPluginContext context) {
		...
		JDialog dialog = new JDialog(owner, "Rename/Move", JDialog.ModalityType.APPLICATION_MODAL);
		...
		NuclrPluginCallback callback = new NuclrPluginCallback() {
			...
			public void onStart(String description) {
				SwingUtilities.invokeLater(() -> itemLabel.setText("Moving " + (description == null ? "" : description)));
			}
			...
			public void onError(String description, Exception e) {
				log.warn("Move error for [{}]: {}", description, e == null ? "?" : e.getMessage());
			}
		...
		Thread.ofVirtual().start(() -> {
			try {
				...
				log.error("Move work failed: {}", t.getMessage(), t);
	...
	private static void runOnEdtAndWait(Runnable runnable) {
		...
		try {
			...
			log.warn("Failed to run move progress dialog on EDT: {}", e.getMessage(), e);
		}
	}
}
